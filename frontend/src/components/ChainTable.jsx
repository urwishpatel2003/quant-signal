export default function ChainTable({ chain, selectedExpiry, livePrice }) {
  ...
  return (
    <div className="card fade-in">
      <div style={{ fontSize: 10, color: '#444', marginBottom: 2 }}>ATM CHAIN — {selectedExpiry}</div>
      <div style={{ fontSize: 9, color: '#334', marginBottom: 6 }}>near ${livePrice?.toFixed(2)} | STRIKE BID ASK MID IV Δ OI</div>
        ...
        <div key={label} style={{ marginBottom: 8 }}>
          ...
          {contracts?.slice(0, 5).map((c, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '45px 35px 35px 38px 35px 38px 35px', gap: 2, fontSize: 9, padding: '3px 0', borderBottom: `1px solid ${bg}` }}>
              <span style={{ color: c.inTheMoney ? (label === 'CALLS' ? '#00ff88' : '#ff4444') : '#667' }}>${c.strike}</span>
              <span style={{ color: '#ff6666' }}>${c.bid}</span>
              <span style={{ color: '#66ff88' }}>${c.ask}</span>
              <span style={{ color: '#ffaa00', fontWeight: 600 }}>${c.mid}</span>
              <span style={{ color: '#556' }}>{c.iv}%</span>
              <span style={{ color: '#445' }}>{c.delta}</span>
              <span style={{ color: '#334' }}>{c.oi > 999 ? `${(c.oi / 1000).toFixed(1)}k` : c.oi}</span>
            </div>
          ))}
        </div>
      ))}
