...
function PnlBadge({ value, pct, size = 'sm', currency = '$', isInr = false }) {
  if (value == null) return <span style={{ color: '#99aacc' }}>—</span>;
  const pos  = value >= 0;
...
export default function SimulatorTab({ market = 'US' }) {
  ...
  if (!isLoaded || loading) return (
    <div style={{ textAlign: 'center', padding: '60px 0', color: '#c8d8f0', fontSize: 'var(--fs-lg)' }}>
      Loading simulator...
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 40 }}>
          ...
          return (
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  padding: '6px 14px', fontSize: 'var(--fs-body)', cursor: hasReset ? 'not-allowed' : 'pointer',
              ...
              {lastReset && (
                <div style={{ fontSize: 'var(--fs-sm)', color: '#99aacc', textAlign: 'center', marginTop: 2 }}>
      ...
      {resetError && (
        <div style={{ fontSize: 'var(--fs-md)', color: '#ff444488', background: '#ff444411', border: '1px solid #ff444433', borderRadius: 4, padding: '8px 12px' }}>
          {resetError}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, borderBottom: '1px solid #2a2a3e', paddingBottom: 0 }}>
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            padding: '8px 14px', fontSize: 'var(--fs-md)', fontFamily: 'inherit', fontWeight: 700,
                ...
                return (
                  <div key={pos.id} style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 12 }}>
                      {[
                        ...(pos.position_type === 'OPTION' ? [
                          ...
                          { label: 'ENTRY PREM',  value: `${sym}${pos.premium?.toFixed(2)}`, color: '#c8d8f0' },
                          { label: 'BREAK EVEN',  value: pos.option_type === 'CALL'
                              ? `${sym}${(pos.strike + pos.premium).toFixed(2)}`
                              : `${sym}${(pos.strike - pos.premium).toFixed(2)}`, color: '#ffaa00aa' },
                          ...
                          { label: 'ENTRY',   value: `${sym}${pos.entry_price.toFixed(2)}`, color: '#c8d8f0' },
                        ...
                        <div key={label} style={{ background: '#0a0a14', borderRadius: 5, padding: '6px 8px' }}>
                          <div style={{ fontSize: 'var(--fs-xs)', color: '#99aacc', letterSpacing: '0.06em', marginBottom: 3 }}>{label}</div>
                          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 700, color, fontFamily: "'JetBrains Mono', monospace" }}>{value}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0 0' }}>
                      {hitTarget ? (
                        <div style={{ fontSize: 'var(--fs-lg)', color: '#00ff88', background: '#00ff8811',
                        ...
                        <div style={{ fontSize: 'var(--fs-body)', color: '#99aacc', flex: 1 }}>
          ...
          {closedPositions.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                return (
                  <div key={pos.id} style={{
                    ...
                    <div style={{ minWidth: 60 }}>
                      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 15, color: '#e8e8f0' }}>{pos.ticker}</div>
                      <div style={{ fontSize: 'var(--fs-md)', color: pos.direction === 'LONG' ? '#00ff8866' : '#ff444466' }}>{pos.direction}</div>
                    </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 'var(--fs-lg)', color: '#99aacc' }}>
