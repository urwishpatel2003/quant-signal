    ...
    <div style={{
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 'var(--fs-body)', color: isOpen ? '#ffaa00' : '#b0c0dd', fontWeight: 700, letterSpacing: '0.15em', marginBottom: isOpen ? 0 : 2 }}>
            {label}
          </div>
          {!isOpen && preview && (
            <div style={{ fontSize: 'var(--fs-body)', color: '#c8d8f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {preview}
            </div>
          )}
        </div>
        <div style={{ fontSize: 'var(--fs-md)', color: isOpen ? '#ffaa00' : '#7788aa', marginLeft: 10, flexShrink: 0 }}>
          {isOpen ? '▲' : '▼'}
        </div>
      </div>
      {isOpen && (
    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: 'var(--fs-lg)', fontWeight: 600, color: pctColor, marginTop: 2 }}>
            {item.changePct != null
        ...
        <div style={{ textAlign: 'center' }}>
          {scanning ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              ...
              <div style={{ fontSize: 'var(--fs-md)', color: '#ffaa0088', letterSpacing: '0.05em' }}>ANALYZING</div>
            </div>
          ) : scanData?.analysis ? (
            <div style={{ background: sigColor + '11', border: `1px solid ${sigColor}44`, padding: '5px 6px', borderRadius: 2 }}>
              <div style={{ fontSize: 'var(--fs-md)', fontWeight: 700, color: sigColor, lineHeight: 1 }}>{scanData.analysis.signal}</div>
              <div style={{ fontSize: 'var(--fs-md)', color: sigColor + '88', marginTop: 2 }}>{scanData.analysis.confidence}% · LT</div>
            </div>
          ) : (
            <div style={{ fontSize: 'var(--fs-md)', color: '#3a3a5e' }}>—</div>
          )}
        </div>
          ...
          {!isIndia && (
            <button
              ...
              style={{
                ...
                padding: '6px 10px', fontSize: 'var(--fs-body)', fontFamily: 'inherit', transition: 'all 0.15s',
              }}
        ...
        {/* Expand chevron */}
        <div style={{ fontSize: 'var(--fs-md)', color: expanded ? '#ffaa00' : '#3a3a5e', textAlign: 'center' }}>
          {expanded ? '▲' : '▼'}
        </div>
      </div>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 14px', borderTop: '1px solid #1e1e2e' }}>
          {scanError && <div style={{ fontSize: 'var(--fs-lg)', color: '#ff4444', padding: '10px 0' }}>{scanError}</div>}
          {scanning && (
            <div style={{ fontSize: 'var(--fs-lg)', color: '#ffaa0088', padding: '12px 0', textAlign: 'center', letterSpacing: '0.15em' }}>
          ...
          {scanData && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 12 }}>
              {scanData.analysis && (
                <div style={{ fontSize: 'var(--fs-md)', color: '#d0d8f0', lineHeight: 1.7, borderLeft: `2px solid ${sigColor}44`, paddingLeft: 10, fontStyle: 'italic', marginBottom: 4 }}>
              ...
              >
                <div style={{ paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {scanData.ta && (
                    <div>
                      <div style={{ fontSize: 'var(--fs-md)', color: '#4488ff', letterSpacing: '0.15em', fontWeight: 700, marginBottom: 8 }}>TECHNICAL</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                        {[
                          ...
                          <div key={k} style={{ background: '#0a0a12', padding: '6px 8px', borderRadius: 3 }}>
                            <div style={{ fontSize: 'var(--fs-sm)', color: '#c8d8f0', marginBottom: 2, letterSpacing: '0.1em' }}>{k}</div>
                  ...
                  {scanData.fundamentals && (
                    <div>
                      <div style={{ fontSize: 'var(--fs-md)', color: '#ffaa00', letterSpacing: '0.15em', fontWeight: 700, marginBottom: 8 }}>FUNDAMENTALS</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                        {[
                          ...
                          <div key={k} style={{ background: '#0a0a12', padding: '6px 8px', borderRadius: 3 }}>
                            <div style={{ fontSize: 'var(--fs-sm)', color: '#c8d8f0', marginBottom: 2, letterSpacing: '0.1em' }}>{k}</div>
                            <div style={{ fontSize: 'var(--fs-md)', fontWeight: 700, color: '#e8e8f0' }}>{v}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {scanData.analysis && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <div style={{ background: '#070e0a', border: '1px solid #00ff8822', borderRadius: 4, padding: '10px' }}>
                        <div style={{ fontSize: 'var(--fs-md)', color: '#00ff88', fontWeight: 700, marginBottom: 8, letterSpacing: '0.1em' }}>▲ BULL</div>
                        {scanData.analysis.bullFactors?.slice(0, 3).map((f, i) => (
                          <div key={i} style={{ fontSize: 'var(--fs-body)', color: '#d0d8f0', padding: '3px 0', borderBottom: i < 2 ? '1px solid #0f1a14' : 'none', display: 'flex', gap: 5, lineHeight: 1.4 }}>
                      ...
                      <div style={{ background: '#0e0707', border: '1px solid #ff444422', borderRadius: 4, padding: '10px' }}>
                        <div style={{ fontSize: 'var(--fs-md)', color: '#ff4444', fontWeight: 700, marginBottom: 8, letterSpacing: '0.1em' }}>▼ BEAR</div>
                        {scanData.analysis.bearFactors?.slice(0, 3).map((f, i) => (
                          <div key={i} style={{ fontSize: 'var(--fs-body)', color: '#d0d8f0', padding: '3px 0', borderBottom: i < 2 ? '1px solid #1a0f0f' : 'none', display: 'flex', gap: 5, lineHeight: 1.4 }}>
              ...
              >
                <div style={{ paddingTop: 10 }}>
                  {scanData.news?.length > 0 ? (
                    scanData.news.slice(0, 6).map((n, i) => (
                      <div key={i} style={{ padding: '8px 0', borderBottom: i < Math.min(scanData.news.length, 6) - 1 ? '1px solid #1a1a26' : 'none' }}>
                        <a href={n.url} target="_blank" rel="noopener noreferrer"
                          style={{ color: '#c8d8f0', fontSize: 'var(--fs-lg)', lineHeight: 1.5, display: 'block', textDecoration: 'none', marginBottom: 3 }}
                          onMouseEnter={e => { e.currentTarget.style.color = '#ffaa00'; }}
                          onMouseLeave={e => { e.currentTarget.style.color = '#c8d8f0'; }}
                        >{n.title}</a>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          <span style={{ color: '#c8d8f0', fontSize: 'var(--fs-md)' }}>{n.publisher}</span>
                          <span style={{ color: '#c8d8f0', fontSize: 'var(--fs-md)' }}>
                    ...
                    <div style={{ fontSize: 'var(--fs-lg)', color: '#c8d8f0', padding: '8px 0' }}>No recent news available</div>
                  )}
                </div>
              </AccordionCard>
            </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <div>
          ...
          <div style={{ fontSize: 'var(--fs-body)', color: '#c8d8f0', marginTop: 2 }}>
        ...
        <button onClick={() => fetchWatchlist(true)} disabled={refreshing}
          style={{ background: 'none', border: '1px solid #2a2a40', color: refreshing ? '#7788aa' : '#b0c0dd', cursor: refreshing ? 'default' : 'pointer', borderRadius: 4, padding: '6px 12px', fontSize: 'var(--fs-lg)', fontFamily: 'inherit', letterSpacing: '0.1em' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          <span style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#ffaa00', fontSize: 'var(--fs-body)' }}>
            {currency}
          </span>
          <input
            value={inputVal}
            ...
            className="input"
            style={{ padding: '10px 12px 10px 26px', fontSize: 'var(--fs-body)', fontWeight: 600, opacity: isFull ? 0.5 : 1 }}
      ...
      {error && <div style={{ fontSize: 'var(--fs-lg)', color: '#ff4444', marginBottom: 10 }}>{error}</div>}

      {!isPro && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <span style={{ fontSize: 'var(--fs-body)', color: '#c8d8f0', letterSpacing: '0.1em' }}>CAPACITY</span>
            <span style={{ fontSize: 'var(--fs-body)', color: isFull ? '#ff4444' : '#b0c0dd' }}>{items.length}/{FREE_LIMIT}</span>
          </div>
          ...
          {isFull && (
            <div style={{ fontSize: 'var(--fs-lg)', color: '#ff4444', marginTop: 5 }}>
              Limit reached —{' '}
              <button onClick={() => setShowUpgrade(true)} style={{ background: 'none', border: 'none', color: '#ffaa00', cursor: 'pointer', fontSize: 'var(--fs-lg)', fontFamily: 'inherit', padding: 0, textDecoration: 'underline' }}>
                upgrade to Pro
      ...
      {loading && <div style={{ textAlign: 'center', padding: '40px 0', fontSize: 'var(--fs-md)', color: '#c8d8f0' }}>LOADING WATCHLIST...</div>}
      ...
      {!loading && items.length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: 48 }}>
          ...
          <div style={{ fontSize: 'var(--fs-lg)', color: '#c8d8f0', lineHeight: 1.8 }}>
            {isIndia
      ...
      {!isPro && items.length > 0 && (
        <div style={{ marginTop: 16, padding: '12px 16px', background: '#ffaa0008', border: '1px solid #ffaa0022', borderRadius: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          <div>
            <div style={{ fontSize: 'var(--fs-lg)', color: '#ffaa00', fontWeight: 700, letterSpacing: '0.1em' }}>UPGRADE TO PRO</div>
            <div style={{ fontSize: 'var(--fs-lg)', color: '#c8d8f0', marginTop: 2 }}>Unlimited watchlist + unlimited scans & options</div>
          </div>
          <button className="btn" onClick={() => setShowUpgrade(true)} style={{ fontSize: 'var(--fs-lg)', padding: '8px 20px' }}>$5/MO →</button>
        </div>
      )}
    </div>
  );
