...
function StatCard({ label, value, sub, color = '#ffaa00' }) {
  return (
    <div style={{ background:'#0f0f1a', border:`1px solid ${color}22`,
      ...
      <div style={{ fontSize: 'var(--fs-body)', color:'#8899bb', letterSpacing:'.1em', marginBottom:6 }}>{label}</div>
      <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color, lineHeight:1 }}>{value ?? '—'}</div>
      {sub && <div style={{ fontSize: 'var(--fs-lg)', color:'#b8c8e0', marginTop:4 }}>{sub}</div>}
    </div>
  );
}
...
export default function PublicAccuracyTab() {
  ...
  if (loading) return (
    <div style={{ textAlign:'center', padding:'60px 0', color:'#b8c8e0' }}>
      <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22,
        color:'#ffaa00', marginBottom:10 }}>LOADING SIGNAL TRACKER</div>
      <div style={{ fontSize: 'var(--fs-md)' }}>Fetching system signals...</div>
    </div>
  );

  if (error) return (
    <div style={{ padding:20, color:'#ff4444', fontSize: 'var(--fs-body)' }}>Error: {error}</div>
  );
    ...
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start',
        ...
        <div>
          ...
          <div style={{ fontSize: 'var(--fs-lg)', color:'#8899bb', marginTop:2 }}>
        ...
        <div style={{ display:'flex', border:'1px solid #2a2a3e', borderRadius:4, overflow:'hidden' }}>
          {[['US','🇺🇸 US'],['INDIA','🇮🇳 INDIA']].map(([key,label]) => (
            <button key={key} onClick={() => setMarket(key)} style={{
              ...
              fontSize: 'var(--fs-lg)', letterSpacing:'.08em', fontWeight:700 }}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',
          ...
          {Object.entries(byTf).map(([tf, d]) => (
            <div key={tf} style={{ background:'#0f0f1a', border:'1px solid #1a1a2e',
              ...
              <div style={{ fontSize: 'var(--fs-body)', color:'#8899bb', letterSpacing:'.08em', marginBottom:6 }}>
                {TF_LABELS[tf] || tf}
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <span style={{ fontSize: 'var(--fs-lg)', color:'#8899bb' }}>{d.total} resolved</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        <span style={{ fontSize: 'var(--fs-lg)', color:'#8899bb' }}>FILTER:</span>
        {/* Timeframe filter */}
        <div style={{ display:'flex', gap:4 }}>
          {[['all','All TF'],['short','Short'],['swing','Swing'],['position','Position'],['longterm','Long']].map(([k,l]) => (
            <button key={k} onClick={() => { setTfFilter(k); setPage(0); }} style={{
              ...
              fontFamily:"'IBM Plex Mono',monospace", fontSize: 'var(--fs-body)', borderRadius:3 }}>
              {l}
            </button>
          ))}
        </div>
        ...
        <span style={{ fontSize: 'var(--fs-lg)', color:'#8899bb', marginLeft:'auto' }}>
          {filtered.length} signals
        </span>
      </div>

      {/* Signals table */}
      <div style={{ background:'#0a0a14', border:'1px solid #1a1a2e', borderRadius:6, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
        <div style={{ minWidth:640 }}>
          {/* Header */}
          <div style={{ display:'grid', gridTemplateColumns:'80px 90px 70px 60px 70px 70px 80px 80px 1fr', gap:0, padding:'8px 12px', borderBottom:'1px solid #1a1a2e', fontSize:'var(--fs-xs)', color:'#8899bb', letterSpacing:'.12em', background:'#0c0c18' }}>
          ...
          {paginated.length === 0 ? (
            <div style={{ padding:'32px 16px', textAlign:'center', fontSize:'var(--fs-sm)', color:'#8899bb' }}>
              {stats.pending > 0
            ...
            const isEven = i % 2 === 0;
            const pctColor = s.outcome_pct == null ? '#7788aa' : s.outcome_pct > 0 ? '#00ff88' : s.outcome_pct < 0 ? '#ff4444' : '#ffaa00';
            return (
              <div key={s.id} style={{
                display:'grid', gridTemplateColumns:'80px 90px 70px 60px 70px 70px 80px 80px 1fr',
                ...
                <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:15, color:'#ffaa00' }}>{s.ticker}</span>
                <span style={{ color:'#7788aa', fontSize:'var(--fs-xs)' }}>
                  {new Date(s.created_at).toLocaleDateString('en-US',{month:'short',day:'numeric'})}
                </span>
                <span style={{ color:'#8899bb', fontSize:'var(--fs-xs)' }}>{s.timeframe?.split(' ')[0] || s.timeframe}</span>
                <span style={{ color: s.signal==='CALL'?'#00ff88':s.signal==='PUT'?'#ff4444': SC[s.signal]||'#fff', fontWeight:700 }}>{s.signal}</span>
                <span style={{ color:'#b0c0dd' }}>{s.confidence}%</span>
                <span style={{ color:'#c8d8f0' }}>{s.market==='INDIA'?'₹':'$'}{s.price_at_signal?.toFixed(2)}</span>
                <span style={{ color:'#7788aa', fontSize:'var(--fs-xs)' }}>
                  {s.price_target ? `${s.market==='INDIA'?'₹':'$'}${s.price_target}` : '—'}
                </span>
                <span style={{ color: s.outcome_result==='WIN'?'#00ff88':s.outcome_result==='LOSS'?'#ff4444':s.outcome_result==='PENDING'?'#556677':'#ffaa00', fontWeight:700, fontSize:'var(--fs-xs)' }}>
                  {s.outcome_result === 'PENDING' ? '⏳' : s.outcome_result}
                </span>
                <span style={{ color: pctColor, fontWeight: s.outcome_pct != null ? 700 : 400 }}>
                  {s.outcome_pct != null ? `${s.outcome_pct > 0?'+':''}${s.outcome_pct.toFixed(1)}%` : '—'}
                  {s.outcome_price && <span style={{ color:'#556677', fontWeight:400, marginLeft:4, fontSize:'var(--fs-xs)' }}>→ {s.market==='INDIA'?'₹':'$'}{s.outcome_price?.toFixed(2)}</span>}
                </span>
              </div>
            );
          })}
      ...
      {totalPages > 1 && (
        <div style={{ display:'flex', justifyContent:'center', gap:8, marginTop:14, alignItems:'center' }}>
          <button onClick={() => setPage(p => Math.max(0,p-1))} disabled={page===0} style={{
            ...
            fontFamily:"'IBM Plex Mono',monospace", fontSize: 'var(--fs-lg)', borderRadius:3 }}>← PREV</button>
          <span style={{ fontSize: 'var(--fs-lg)', color:'#8899bb' }}>{page+1} / {totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages-1,p+1))} disabled={page===totalPages-1} style={{
            padding:'5px 14px', background:'transparent', border:'1px solid #2a2a3e',
            color: page===totalPages-1 ? '#334455' : '#8899bb',
            cursor: page===totalPages-1 ? 'default' : 'pointer',
            fontFamily:"'IBM Plex Mono',monospace", fontSize: 'var(--fs-lg)', borderRadius:3 }}>NEXT →</button>
        </div>
      )}

      {/* Disclaimer */}
      <div style={{ marginTop:20, padding:'12px 16px', background:'#0a0a0f',
        border:'1px solid #1a1a2e', borderRadius:4, fontSize: 'var(--fs-lg)', color:'#b8c8e0', lineHeight:1.7 }}>
