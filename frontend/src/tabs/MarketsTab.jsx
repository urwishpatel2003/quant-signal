...
const REGIONS = [
  { label: '🇯🇵 JAPAN',     syms: [{ name: 'Nikkei 225', sym: '^N225',     fmt: v => v?.toLocaleString('en-US', { maximumFractionDigits: 0 }) }] },
  { label: '🇨🇳 CHINA / HK', syms: [
      { name: 'Shanghai',  sym: '000001.SS', fmt: v => v?.toLocaleString('en-US', { maximumFractionDigits: 2 }) },
      { name: 'Hang Seng', sym: '^HSI',      fmt: v => v?.toLocaleString('en-US', { maximumFractionDigits: 0 }) },
    ]
  },
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ fontSize: 10, color: up ? '#00ff88' : '#ff4444' }}>{up ? '▲' : '▼'} {Math.abs(data.changePct)?.toFixed(2)}%</div>
      </div>
    </div>
  );
}
...
export default function MarketsTab({ intlMarkets, bonds, macroNews, calendar }) {
  ...
  const globalSentiment = upCount >= 5 ? 'RISK ON' : upCount <= 2 ? 'RISK OFF' : 'MIXED';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="markets-grid">
        ...
        {bonds && (
          <div className="card">
            <div style={{ fontSize: 10, color: '#ffaa0066', letterSpacing: '0.15em', marginBottom: 10 }}>📊 US BONDS</div>
            {[
              ['10Y Yield',   `${bonds.tnx?.current?.toFixed(2)}%`, bonds.tnx?.changePct > 0 ? '#ff4444' : '#00ff88'],
              ['2Y Yield',    `${bonds.irx?.current?.toFixed(2)}%`, '#c8c8d0'],
              ['30Y Yield',   `${bonds.tyx?.current?.toFixed(2)}%`, '#c8c8d0'],
              ['TLT (20Y)',   `$${bonds.tlt?.current?.toFixed(2)}`, bonds.tlt?.changePct > 0 ? '#00ff88' : '#ff4444'],
          ...
          <div className="card">
            ...
            {macroNews.slice(0, 8).map((n, i) => (
              <div key={i} style={{ padding: '5px 0', borderBottom: '1px solid #1a1a26', fontSize: 10 }}>
                <div style={{ color: '#ffaa0055', fontSize: 9, marginBottom: 2 }}>{n.topic?.split(' ').slice(0, 3).join(' ')?.toUpperCase()}</div>
