...
const empty = {
  ticker: '', type: 'CALL', strike: '', expiry: '',
  entry: '', exit: '', contracts: '1', notes: '', date: ''
};
    ...
    <div>
      ...
      {trades.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 8, marginBottom: 16 }}>
          {[
            ['TOTAL TRADES',  trades.length,                    '#c8c8d0'],
            ['CLOSED',        closed.length,                    '#c8c8d0'],
            ['WIN RATE',      `${winRate}%`,                    wrColor],
            ['TOTAL P&L',     `${totalPL >= 0 ? '+' : ''}$${totalPL.toFixed(0)}`, plColor],
      ...
      {adding && (
        <div className="card" style={{ marginBottom: 16, borderColor: '#ffaa0044' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {['CALL', 'PUT'].map(t => (
                  <button key={t} className="btn-sm"
                    style={{ flex: 1, color: form.type === t ? (t === 'CALL' ? '#00ff88' : '#ff4444') : '#556', borderColor: form.type === t ? (t === 'CALL' ? '#00ff88' : '#ff4444') : '#2a2a3e', background: form.type === t ? (t === 'CALL' ? '#00ff8811' : '#ff444411') : '#1a1a2e' }}
                    onClick={() => set('type', t)}>
                    {t}
                  </button>
                ))}
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 9, color: '#445', marginBottom: 4 }}>NOTES / REASON FOR TRADE</div>
            <textarea
              value={form.notes}
      ...
      {trades.length === 0 && !adding && (
        <div className="card" style={{ textAlign: 'center', padding: 60, color: '#333' }}>
          ...
          <div style={{ fontSize: 11, marginBottom: 16 }}>Click "+ LOG TRADE" to start tracking your performance</div>
              ...
              <div key={t.id} className="card" style={{ borderColor: t.pl === null ? '#1e1e2e' : (t.pl >= 0 ? '#00ff8822' : '#ff444422') }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ minWidth: 90 }}>
                    ...
                    <div style={{ fontSize: 9, color: '#334', marginTop: 2 }}>{t.date}</div>
                  </div>
                    ...
                    <div style={{ fontSize: 12, color: '#c8c8d0' }}>
                      ...
                      {t.exit
                        ...
                        : <input
                            ...
                            className="input"
                            style={{ width: 70, padding: '2px 6px', fontSize: 11, display: 'inline-block' }}
                          />
                      }
                    </div>
                    <div style={{ fontSize: 9, color: '#445', marginTop: 2 }}>{t.contracts} contract{t.contracts > 1 ? 's' : ''}</div>
                  </div>
                  ...
                  {t.win !== null && (
                    <div style={{
                      ...
                      border: `1px solid ${t.win ? '#00ff8844' : '#ff444444'}`,
                      borderRadius: 2
                    }}>
                      {t.win ? '✓ WIN' : '✗ LOSS'}
                    </div>
                  ...
                  {/* Notes */}
                  {t.notes && (
                    <div style={{ flex: 1, fontSize: 11, color: '#667', fontStyle: 'italic', minWidth: 150 }}>
                      "{t.notes}"
                    </div>
                  )}
                  ...
                  <button
                    ...
                    style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#333', cursor: 'pointer', fontSize: 16, padding: '4px 8px' }}
                    title="Delete trade">
                    ×
                  </button>
                </div>
